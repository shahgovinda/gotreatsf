...
interface CartStore {
    ...
    // ✅ RENAMED 'taxRate' to 'packagingCharge'
    calculateTotalPrice: (deliveryCharge: number, packagingCharge: number) => void
    ...
    devtools(
        persist(
            (set, get) => ({
                ...
                calculateTotalPrice: (deliveryCharge, packagingCharge) => set((state) => {
                    
                    // 1. Calculate base total after item discount
                    const discountedGross = Math.max(0, state.grossTotalPrice - state.voucherDiscount);
                    
                    // 2. Add fixed charges (delivery and packaging)
                    let finalTotal = discountedGross + deliveryCharge + packagingCharge;
                    
                    // 3. Format to two decimal places
