...
    // Create a list of available dates (e.g., today and the next 2 days)
...
        },
        {
            label: format(addDays(new Date(), 2), 'EEEE, MMM d'),
...
            setPreferredDeliveryDate(availableDates[0].value);
        }
    }, [availableDates, preferredDeliveryDate]);
...
                note: note,
                // Add deliveryDate to the orderDetails
...
                    customer_Note: note || '',
                    delivery_Date: preferredDeliveryDate, // Add delivery date to notes
...
                        
                        {/* New section for Preferred Delivery Date */}
...
                                        <input
                                            type="radio"
                                            id={`delivery-date-${idx}`}
                                            name="deliveryDate"
                                            value={date.value}
                                            checked={preferredDeliveryDate === date.value}
                                            onChange={() => setPreferredDeliveryDate(date.value)}
                                            className="sr-only peer"
                                        />
                                        <label
                                            htmlFor={`delivery-date-${idx}`}
...
                                    "6:00 PM - 8:00 PM",
                                    "8:00 PM - 10:00 PM",
                                    // Correcting the time slot based on the user's image
                                    "10:00 PM - 1:00 AM",
                                ].map((slot, idx) => (
                                    <div key={idx} className="relative">
                                        <input
...
                                            name="deliveryTime"
                                            value={slot}
                                            checked={preferredDeliveryTime === slot.split(' ')[0] && preferredDeliveryPeriod === slot.split(' ').slice(-1)[0]}
                                            onChange={() => {
                                                setDeliveryTime(slot.split(' ')[0]);
                                                setDeliveryPeriod(slot.split(' ').slice(-1)[0]);
                                            }}
                                            className="sr-only peer"
                                        />
                                        <label
...
                                            className={`p-4 rounded-xl border-2 text-sm font-medium transition-all text-center flex-grow cursor-pointer
                                                ${(preferredDeliveryTime === slot.split(' ')[0] && preferredDeliveryPeriod === slot.split(' ').slice(-1)[0])
                                                ? "border-orange-500 bg-orange-50 text-orange-700 shadow"
                                                : "border-gray-200 bg-white hover:border-orange-400 hover:bg-orange-50"
                                            }`}
                                        >
