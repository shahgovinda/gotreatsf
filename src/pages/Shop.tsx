...
    const [priceFilter, setPriceFilter] = useState<number | null>(null); // New state for price filter
...
        setPriceFilter(prev => prev === price ? null : price);
    };
...
            // No specific tag filter for top-picks, already filtered by foodType
        } else if (tag === 'meals') {
            filteredProducts = filteredProducts?.filter(item => item.category === 'Meals');
        } else if (tag === 'pasta') {
            filteredProducts = filteredProducts?.filter(item => item.category === 'Pasta');
        } else if (tag === 'maggi') {
            filteredProducts = filteredProducts?.filter(item => item.category === 'Maggi');
        } else if (tag === 'paav-bhaaji') {
            filteredProducts = filteredProducts?.filter(item => item.category === 'Paav Bhaaji');
        } else if (tag === 'desserts') {
            filteredProducts = filteredProducts?.filter(item => item.category === 'Desserts');
        } else if (tag === 'snacks') {
            filteredProducts = filteredProducts?.filter(item => item.category === 'Snacks');
        } else if (tag === 'drinks') {
            filteredProducts = filteredProducts?.filter(item => item.category === 'Drinks');
        } else if (tag === 'pickles') {
            filteredProducts = filteredProducts?.filter(item => item.category === 'Pickles');
        }

        // Finally, filter by price range
        if (priceFilter !== null) {
            filteredProducts = filteredProducts?.filter(item => item.productPrice < priceFilter);
        }
        setFilteredItemCount(filteredProducts?.length || 0);
...
                    {/* Price Filters */}
                    <div className='flex justify-center items-center flex-wrap gap-2 lg:gap-5 mt-5 select-none px-4'>
                        <h3 className='font-semibold text-gray-700 whitespace-nowrap'>Filter by Price:</h3>
                        <div className='flex flex-wrap gap-2'>
                            {[50, 100, 150, 200, 300, 500].map(price => (
                                <button
                                    key={price}
                                    onClick={() => handlePriceFilter(price)}
                                    className={`whitespace-nowrap px-4 py-2 rounded-lg transition-colors duration-100 ease-in ${priceFilter === price ? 'bg-orange-600 text-white' : 'bg-white text-gray-700 hover:bg-orange-100'}`}
                                >
                                    Under ₹{price}
                                </button>
                            ))}
...
                    {/* Pop-up message for search results */}
                    {showPopup && searchQuery && (
                        <motion.div
                            initial={{ opacity: 0, y: -20 }}
                            animate={{ opacity: 1, y: 0 }}
                            exit={{ opacity: 0, y: -20 }}
                            className="fixed top-20 left-1/2 -translate-x-1/2 bg-green-500 text-white px-6 py-3 rounded-full shadow-lg z-50 text-center"
                        >
                            <p className="font-semibold">{filteredItemCount} items found for "{searchQuery}"</p>
                        </motion.div>
                    )}
...
                            <button
                                onClick={() => { setSearchQuery(''); setPriceFilter(null); setFoodType('all'); }}
